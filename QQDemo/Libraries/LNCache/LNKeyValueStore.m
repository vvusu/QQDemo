...
- (void)createTableWithName:(NSString *)tableName {
    if ([LNKeyValueStore checkTableName:tableName] == NO) {
        return;
    }
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    LNKeyValueItem * item = [self getLNKeyValueItemById:objectId fromTable:tableName];
    if (item) {
...
- (LNKeyValueItem *)getLNKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    if (json) {
        ...
        LNKeyValueItem * item = [[LNKeyValueItem alloc] init];
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (LNKeyValueItem * item in result) {
        error = nil;
...
- (void)putDataObject:(id)object withId:(NSString *)objectId intoTable:(NSString *)tableName {
    ...
    __block BOOL result;
    [_dbQueue inDatabase:^(FMDatabase *db) {
        result = [db executeUpdate:sql, objectId, object, createdTime];
    }];
    if (!result) {
        debugLog(@"ERROR, failed to insert/replace into table: %@", tableName);
    }
...
- (id)getDataObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    LNKeyValueItem * item = [self getLNKeyValueDataItemById:objectId fromTable:tableName];
    if (item) {
        return item.itemObject;
    } else {
        return nil;
...
- (LNKeyValueItem *)getLNKeyValueDataItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    if ([LNKeyValueStore checkTableName:tableName] == NO) {
        return nil;
    }
    NSString * sql = [NSString stringWithFormat:QUERY_ITEM_SQL, tableName];
